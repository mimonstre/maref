...
import CompareFamilySection from "@/features/compare/CompareFamilySection";
import {
...
import type { CompareGroup } from "@/features/compare/types";
import { getOfferCompareFamily } from "@/features/compare/families";
...
export default function ComparePageClient() {
  ...
  const multiFamily = groupsWithOffers.length > 1;
  ...
  function handleAddOffer(offer: Offer) {
    ...
    setCompareGroups(loadCompareGroups());
    setSearch("");
  ...
  function handleRemoveOffer(groupKey: string, offerId: string) {
    setCompareGroups(removeOfferFromCompareGroup(groupKey, offerId));
  }

  function handleClearGroup(groupKey: string) {
    setCompareGroups(clearCompareGroup(groupKey));
    showMessage("Comparaison retiree.");
  }
    ...
    <div className="space-y-6">
      ...
      <section className="premium-hero rounded-[32px] px-6 py-7 text-white md:px-8 md:py-8">
        <div className="relative z-10 flex items-end justify-between gap-4">
          <div>
            <p className="text-[0.72rem] font-bold uppercase tracking-[0.22em] text-blue-100">Comparateur premium</p>
            <h2 className="section-title mt-2 text-3xl font-black tracking-tight md:text-4xl">Comparaison par famille</h2>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-blue-100/90">
              Les offres sont rangees par famille pour garder des arbitrages propres, lisibles et visuellement plus solides.
            </p>
          </div>
      ...
      <div className="premium-surface rounded-[30px] p-5">
        <h3 className="section-title text-2xl font-black text-slate-950">Comparaisons actives</h3>
        <p className="mt-2 text-sm text-slate-500">
          {groupsWithOffers.length} comparaison{groupsWithOffers.length > 1 ? "s" : ""} active{groupsWithOffers.length > 1 ? "s" : ""}
      ...
      {projectContext && (
        <div className="premium-surface rounded-[30px] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">{projectContext.projectObjective || "Aucun objectif precise."}</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-800">{projectContext.projectCategory}</span>
            {projectContext.projectBudget && (
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Budget {projectContext.projectBudget}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="premium-surface rounded-[30px] p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[0.72rem] font-bold uppercase tracking-[0.22em] text-blue-700">Comparateur multi-familles</p>
            <h3 className="section-title mt-2 text-2xl font-black text-slate-950">Ajoutez des offres et MAREF les range par famille</h3>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Un lave-linge ne se melange plus avec une TV. Chaque famille garde sa propre comparaison sur cette page.
            </p>
          </div>
        ...
        <div className="mt-4 flex items-center gap-2 rounded-[24px] border border-gray-200 bg-white px-4 py-3 shadow-sm transition-all focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100">
          <Search className="h-5 w-5 shrink-0 text-gray-400" />
          <input
            type="text"
            placeholder="Ajouter une offre a une comparaison..."
        ...
        {search && (
          <div className="mt-2 max-h-64 overflow-y-auto rounded-[24px] border border-gray-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.10)]">
            {filtered.length === 0 ? (
              <p className="p-3 text-sm text-gray-400">Aucun resultat</p>
            ) : (
              filtered.slice(0, 8).map((offer) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 border-b border-gray-100 p-3 text-left transition-colors last:border-0 hover:bg-slate-50"
                  >
                    <span className="text-lg">{family.label}</span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium">{offer.brand} {offer.product}</p>
                      <p className="text-xs text-gray-400">{family.label} - {offer.merchant} - {offer.price.toLocaleString("fr-FR")} EUR</p>
                    </div>
      ...
      {groupsWithOffers.length === 0 && !search && (
        <EmptyState
          icon={<Layers3 className="h-8 w-8 text-gray-400" />}
          title="Aucune comparaison en cours"
          description="Ajoutez des offres depuis l explorer ou les fiches produit. Elles seront classees automatiquement par famille."
        />
      )}
      ...
      {multiFamily && (
        <div className="grid grid-cols-3 gap-3">
          <div className="premium-card rounded-[24px] p-4 text-center">
            <p className="text-2xl font-black text-blue-700">{groupsWithOffers.length}</p>
            <p className="text-[0.68rem] uppercase tracking-[0.18em] text-slate-500">Familles actives</p>
          </div>
          <div className="premium-card rounded-[24px] p-4 text-center">
            <p className="text-2xl font-black text-blue-700">{readyFamilies}</p>
            <p className="text-[0.68rem] uppercase tracking-[0.18em] text-slate-500">Comparaisons pretes</p>
          </div>
          <div className="premium-card rounded-[24px] p-4 text-center">
            <p className="text-2xl font-black text-blue-700">{incompleteFamilies}</p>
            <p className="text-[0.68rem] uppercase tracking-[0.18em] text-slate-500">Familles a completer</p>
          </div>
        </div>
      )}
      ...
      {incompleteFamilies > 0 && (
        <IncompleteDataWarning
          title="Certaines familles sont encore incompletes"
          description="Une comparaison devient vraiment utile a partir de 2 offres dans la meme famille. Vous pouvez garder plusieurs familles cote a cote sur cette page."
        />
      )}
