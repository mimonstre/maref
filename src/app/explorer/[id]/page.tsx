...
export default function OfferDetailPage() {
  ...
  useEffect(() => {
    async function loadOfferPage() {
      ...
      const currentOffer = await getOfferById(params.id as string);
    ...
    void Promise.resolve().then(loadOfferPage);
  }, [params.id]);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
        <div className="flex gap-4 items-start">
          <div className="w-24 h-24 rounded-xl bg-gray-50 flex items-center justify-center text-4xl shrink-0">{categoryIcon}</div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          {projectedAnalysis?.averageScore ? <ScoreCircle score={projectedAnalysis.averageScore} size="sm" /> : null}
        </div>
              ...
              <div className="mb-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <MimoCard
                  ...
                  text={
                    ...
                    info.name +
                    " (" +
                    offer.pefas[key as keyof typeof offer.pefas] +
                    "/100) : " +
                    (offer.pefas[key as keyof typeof offer.pefas] >= 75
                      ? "c est un point fort de cette offre."
                      : offer.pefas[key as keyof typeof offer.pefas] >= 55
        ...
        {!hasSpecs ? (
          ...
          <div>
            <div className="p-4 bg-gray-50 border-b border-gray-100">
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.entries(offer.specs).slice(0, 6).map(([key, value]) => (
                ...
                {Object.entries(specCategories).map(([categoryName, items]) => (
                  <div key={categoryName}>
                    ...
                    >
                      ...
                      <svg className={"w-4 h-4 text-gray-400 transition-transform " + (activeSpecCat === categoryName ? "rotate-180" : "")} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    ...
                    {activeSpecCat === categoryName && (
                      <div className="px-4 pb-4">
                        {items.map(([key, value]) => {
                          const isGood =
                            (key.includes("energetique") && (value === "A" || value === "B")) ||
                            (key.includes("sonore") && parseInt(value) <= 42) ||
                            key.includes("No Frost") ||
                            value.includes("No Frost") ||
                            (key.includes("Dolby") && value === "Oui") ||
                            (key === "Vapeur" && value === "Oui");
                          const isBad =
                            (key.includes("energetique") && (value === "F" || value === "G")) ||
            ...
            {showAllSpecs && (
              <div className="p-4 border-t border-gray-100">
                <MimoCard compact text={generateSpecsMimo(offer.specs)} />
              </div>
            )}
          </div>
        )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
        ...
        {(showAllVigilances ? offer.vigilances : offer.vigilances.slice(0, 2)).map((vigilance, index) => (
          <div key={index} className="py-2 border-b border-gray-100 last:border-0 text-sm flex items-center gap-2">
              ...
              <div key={alternative.id} onClick={() => router.push("/explorer/" + alternative.id)} className="bg-white rounded-xl border border-gray-200 p-3 flex gap-3 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="font-bold text-xs">{alternative.price.toLocaleString("fr-FR")} EUR</span>
              ...
              <div className="absolute bottom-14 left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl py-2 z-50 animate-scale-in">
                {projects.length === 0 ? (
                  ...
                  projects.map((project) => (
                    <button key={project.id} onClick={() => addToProject(project.id)} className="block w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors">
